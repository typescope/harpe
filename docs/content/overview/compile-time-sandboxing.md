...
+++
Harpe checks granted authority **before generated code runs**. Every side
effect is denied by default. Code generated by the model can use only the typed
...
permissions) speak in the language of machine resources. They can block a file
or host, but not naturally express “only this customer's rows,” “calendar reads
but no writes,” or “draft a refund but require approval.”
